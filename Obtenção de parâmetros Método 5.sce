...
exec("C:\Users\jvms9\OneDrive\Área de Trabalho\Comparação Al-Ajmi CBTERMO\F1\Pedersen\Função PC-SAFT para obter Z.sce")
...
nr = sqrt((1+2*In)./(1-In)) //p. 66 Riazi, índice de refração a 20°C
...
//Cálculo dos parâmetros m, epsilon_K e sigma da PC-SAFT para cada SCN (Pedersen et al. (2012))
mAC7 = 2.4653
mNC7 = 2.5303
mPC7 = 3.4831
MAC7 = 78.114
...
C7m = xCp*mPC7 + xCn*mNC7 + xCa*mAC7
C7M = xCp*MPC7 + xCn*MNC7 + xCa*MAC7
C7ro = C7M./(xCp*MPC7/roPC7 + xCn*MNC7/roNC7 + xCa*MAC7/roAC7)
C7em = xCp*mPC7*ePC7_K + xCn*mNC7*eNC7_K + xCa*mAC7*eAC7_K

m = C7m + 2.82076e-2*(M./d-C7M./C7ro)
e_K = (C7em + 7.97066*(M.*d.^0.25 - C7M.*C7ro.^0.25))./m

//Cálculo do parâmetro sigma da PC-SAFT para cada MCN
...
for i = 1:length(SG)
    x = 1
    sigmachute = 3.71 //angstrons, chute
    epsilon_K(i) = e_K(i) //K
    k = 0
    function z = g(sigma)
